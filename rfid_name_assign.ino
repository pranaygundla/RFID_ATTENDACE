...
going to be written into the card, stored in flash memory */
const char blockData[16] PROGMEM = "Bharat_Kumar";  // Change the name you want to store in RFID Tag
//-----------------------------------------
...
void setup() 
{
  ...
  mfrc522.PCD_Init();
  Serial.println(F("Scan a RFID Tag to write data..."));
  //-----------------------------------------
}
...
void WriteDataToBlock(int blockNum, const char* blockData) 
{
  //------------------------------------------------------------------------------
  ...
  status = mfrc522.MIFARE_Write(blockNum, (byte*)blockData, 16);
  if (status != MFRC522::STATUS_OK) {
